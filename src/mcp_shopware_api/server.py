...
async def search_shopware_entities(
    ...
    **IMPORTANT - Product Variant Filtering**:
    ...
    try:
        ...
        if entity == "product" and "filter" not in search_criteria:
...
async def shopware_post_request(
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            result = response.json()
            return (
                f"POST request successful for endpoint '{endpoint}'. Result: {result}"
            )
        else:
...
async def get_entity_openapi_schema(entity: str) -> str:
    ...
    try:
        ...
        result = {"entity": entity, "paths": entity_paths, "schemas": entity_schemas}

        return f"OpenAPI schema for entity '{entity}': {json.dumps(result, indent=2)}"
...
async def get_entity_definition(entity: str) -> str:
    ...
    try:
        ...
        if entity_key in schema_data:
            ...
            return f"Entity definition for '{entity}': {json.dumps(result, indent=2)}"
        ...
        return f"Error getting entity definition for '{entity}': {str(e)}"
    ...
    **Error Handling**:
    ...
    try:
        ...
        if not isinstance(payload, list):
            return (
                f"Payload must be a list of objects (for upsert) or IDs (for delete)."
            )
        ...
        if response.status_code in [200, 201]:
            ...
            perf_text = f" (Performance: {', '.join(perf_info)})" if perf_info else ""
